...
class ElasticClient[JsonR](config: ElasticClientConfig)(implicit actorSystem: ActorSystem) extends Elastic[JsonR] {
  ...
  private def indexPath(names: Seq[String]) = names match {
    case Nil | Seq() => Path.Empty
    case _ => Path.Empty / names.mkString(",")
  }
  ...
  override def deleteIndex(name: String)(implicit sReader: Reader[String, JsonR], jsonReader: Reader[JsonR, IndexOps], ec: ExecutionContext): Future[IndexOps] = {
    delete(indexPath(Seq(name)), None).map(str => jsonReader.read(sReader.read(str)))
  }
  ...
  override def putMapping[M](name: String, mapping: M)(implicit mWrites: Writer[M, String], sReader: Reader[String, JsonR], jsonReader: Reader[JsonR, IndexOps], ec: ExecutionContext): Future[IndexOps] = {
    val strMapping = mWrites.write(mapping)
    put(Path.Empty / name / "_mapping", Some(strMapping)).map(str => jsonReader.read(sReader.read(str)))
  }

  override def getMapping(index: String)(implicit sReader: Reader[String, JsonR], ec: ExecutionContext): Future[JsonR] = {
    getMappings(Seq(index))(sReader, ec)
  }

  override def getMappings(index: Seq[String])(implicit sReader: Reader[String, JsonR], ec: ExecutionContext): Future[JsonR] = {
    val indexPath: Path = if (index.isEmpty) Path./ else Path.Empty / index.mkString(",")
    get(indexPath / "_mapping").map(str => sReader.read(str))
  }
  ...
  override def refresh(indexes: String*)(implicit sReader: Reader[String, JsonR], jsonReader: Reader[JsonR, IndexResponse[JsonR]], ec: ExecutionContext): Future[IndexResponse[JsonR]] = {
    post(indexPath(indexes) / "_refresh").map(s => jsonReader.read(sReader.read(s)))
  }
  ...
  override def clearCache(indexes: Seq[String])(implicit sReader: Reader[String, JsonR], ec: ExecutionContext): Future[JsonR] = {
    post(Path.Empty / indexes.mkString(",") / "_cache" / "clear").map(sReader.read)
  }
  ...
  override def mget(index: Option[String] = None, request: MGets)(implicit sWriter: Writer[JsonR, String], jsonWriter: Writer[MGets, JsonR], sReader: Reader[String, JsonR], jsonReader: Reader[JsonR, MGetResponse[JsonR]], ec: ExecutionContext): Future[MGetResponse[JsonR]] = {
    val indexPath = index.map(i => Path.Empty / i).getOrElse(Path.Empty)
    val body = Some(sWriter.write(jsonWriter.write(request)))
    get(indexPath / "_mget", body).map(sReader.read).map(jsonReader.read)
  }

  override def search[Q](index: Seq[String] = Seq.empty[String], query: Q, from: Int = 0, size: Int = 20, search_type: SearchType = QUERY_THEN_FETCH, request_cache: Boolean, terminate_after: Option[Int], timeout: Option[Int])(implicit qWrites: Writer[Q, String], sReader: Reader[String, JsonR], jsonReader: Reader[JsonR, SearchResponse[JsonR]], ec: ExecutionContext): Future[SearchResponse[JsonR]] = {
    val indexPath: Path = if (index.isEmpty) Path./ else Path.Empty / index.mkString(",")
    get(indexPath / "_search", Some(qWrites.write(query))).map(str => jsonReader.read(sReader.read(str)))
  }

  override def bulk[D](index: Option[String], publisher: Publisher[Bulk[D]], batchSize: Int)(implicit qWrites: Writer[JsonR, String], docWriter: Writer[D, JsonR], bulkOpWriter: Writer[BulkOpType, JsonR], sReader: Reader[String, JsonR], bReader: Reader[JsonR, BulkResponse[JsonR]], ec: ExecutionContext): Publisher[BulkResponse[JsonR]] = {
    Source.fromPublisher(publisher)
      .grouped(batchSize)
      .mapAsync(1) { group => oneBulk(index, group)}
      .runWith(Sink.asPublisher(fanout = true))
  }

  override def bulkFlow[D](index: Option[String], batchSize: Int)(implicit sWrites: Writer[JsonR, String], docWriter: Writer[D, JsonR], bulkOpWriter: Writer[BulkOpType, JsonR], sReader: Reader[String, JsonR], bReader: Reader[JsonR, BulkResponse[JsonR]], ec: ExecutionContext): Flow[Bulk[D], BulkResponse[JsonR], NotUsed] = {
    Flow[Bulk[D]].grouped(batchSize).mapAsync(1) { group => oneBulk(index, group)}
  }

  override def oneBulk[D](index: Option[String], bulk: Seq[Bulk[D]])(implicit sWrites: Writer[JsonR, String], docWriter: Writer[D, JsonR], bulkOpWriter: Writer[BulkOpType, JsonR], sReader: Reader[String, JsonR], bReader: Reader[JsonR, BulkResponse[JsonR]], ec: ExecutionContext): Future[BulkResponse[JsonR]] = {
    val indexPath: Option[Path] = index.map(i => Path.Empty / i)
  ...
  override def scroll[Q](index: Seq[String] = Seq.empty, query: Q, scroll: String = "1m", size: Int = 20)(implicit qWrites: Writer[Q, String], jsonWriter: Writer[Scroll, JsonR], sWriter: Writer[JsonR, String], sReader: Reader[String, JsonR], jsonReader: Reader[JsonR, SearchResponse[JsonR]], ec: ExecutionContext): Source[SearchResponse[JsonR], NotUsed] = {
    val path = indexPath(index)
  ...
  override def scrollPublisher[Q](index: Seq[String] = Seq.empty, query: Q, scroll: String = "1m", size: Int = 20)(implicit qWrites: Writer[Q, String], jsonWriter: Writer[Scroll, JsonR], sWriter: Writer[JsonR, String], sReader: Reader[String, JsonR], jsonReader: Reader[JsonR, SearchResponse[JsonR]], ec: ExecutionContext): Publisher[SearchResponse[JsonR]] = {
    val s: Source[SearchResponse[JsonR], NotUsed] = this.scroll(index, query, scroll, size)
    s.runWith(Sink.asPublisher(fanout = true))
  }

  override def aggregation[Q](index: Seq[String], query: Q, from: Option[Int], size: Option[Int], search_type: Option[SearchType], request_cache: Boolean, terminate_after: Option[Int], timeout: Option[Int])(implicit qWrites: Writer[Q, String], respReads: Reader[String, SearchResponse[JsonR]], jsonReader: Reader[String, JsonR], ec: ExecutionContext): Future[SearchResponse[JsonR]] = notImplemented[SearchResponse[JsonR]]

  override def index(name: String): Index[JsonR] = EsIndex(name)

  case class EsIndex(name: String) extends Index[JsonR] {

    private val indexPath = _this.indexPath(Seq(name))

//    override def /: Index[JsonR] = EsIndex(name)
    ...
    override def get(id: String, routing: Option[String], fields: Seq[String], _source: Boolean)(implicit sReader: Reader[String, JsonR], jsonReader: Reader[JsonR, GetResponse[JsonR]], ec: ExecutionContext): Future[GetResponse[JsonR]] = {
      ...
      _this.get(indexPath / "_doc" / id, None, query).map(str => jsonReader.read(sReader.read(str)))
    }
    ...
    override def index[D](data: D, id: Option[String], version: Option[Int], versionType: Option[VersionType], create: Boolean, routing: Option[String], parent: Option[String], refresh: Boolean, timeout: Option[String], consistency: Option[Consistency], detectNoop: Boolean)(implicit writer: Writer[D, JsonR], strWriter: Writer[JsonR, String], sReader: Reader[String, JsonR], jsonReader: Reader[JsonR, IndexResponse[JsonR]], ec: ExecutionContext): Future[IndexResponse[JsonR]] = {
      ...
      id match {
        case Some(_id) if create =>
          put(indexPath / "_create" / _id, Some(strWriter.write(writer.write(data))), query).map(str => jsonReader.read(sReader.read(str)))
        case Some(_id) =>
          put(indexPath / "_doc" / _id, Some(strWriter.write(writer.write(data))), query).map(str => jsonReader.read(sReader.read(str)))
        case None =>
    ...
    override def update[D](data: D, id: String, version: Option[Int], versionType: Option[VersionType], retry_on_conflict: Option[Int], routing: Option[String], parent: Option[String], timeout: Option[String], refresh: Boolean, wait_for_active_shards: Option[Boolean])(implicit writer: Writer[D, JsonR], strWriter: Writer[JsonR, String], sReader: Reader[String, JsonR], jsonReader: Reader[JsonR, IndexResponse[JsonR]], ec: ExecutionContext): Future[IndexResponse[JsonR]] = {
      ...
      val query: Option[Query] = if (querys.isEmpty) None else Some(Query(querys: _*))
      post(indexPath / "_update" / id, Some(strWriter.write(writer.write(data))), query).map(str => jsonReader.read(sReader.read(str)))
    }
    ...
    override def delete(id: String, refresh: Boolean = false, routing: Option[String] = None)(implicit sReader: Reader[String, JsonR], jsonReader: Reader[JsonR, IndexResponse[JsonR]], ec: ExecutionContext): Future[IndexResponse[JsonR]] = {
      ...
      _this.delete(indexPath / "_doc" / id, query = query).map(str => jsonReader.read(sReader.read(str)))
    }
    ...
    override def mget(request: MGets)(implicit sWriter: Writer[JsonR, String], jsonWriter: Writer[MGets, JsonR], sReader: Reader[String, JsonR], jsonReader: Reader[JsonR, MGetResponse[JsonR]], ec: ExecutionContext): Future[MGetResponse[JsonR]] =
      _this.mget(Some(name), request)(sWriter, jsonWriter, sReader, jsonReader, ec)
    ...
    override def search[Q](query: Q)(implicit qWrites: Writer[Q, String], sReads: Reader[String, JsonR], jsonReader: Reader[JsonR, SearchResponse[JsonR]], ec: ExecutionContext): Future[SearchResponse[JsonR]] =
      _this.search[Q](Seq(name), query)(qWrites, sReads, jsonReader, ec)
    ...
    override def delete(implicit sReader: Reader[String, JsonR], jsonReader: Reader[JsonR, IndexOps], ec: ExecutionContext): Future[IndexOps] = {
      println(name)
      _this.deleteIndex(name)(sReader, jsonReader, ec)
    }
    ...
    override def bulk[D](publisher: Publisher[Bulk[D]], batchSize: Int)(implicit qWrites: Writer[JsonR, String], docWriter: Writer[D, JsonR], bulkOpWriter: Writer[BulkOpType, JsonR], sReader: Reader[String, JsonR], bReader: Reader[JsonR, BulkResponse[JsonR]], ec: ExecutionContext): Publisher[BulkResponse[JsonR]] =
      _this.bulk(Some(name), publisher, batchSize)(qWrites, docWriter, bulkOpWriter, sReader, bReader, ec)
    ...
    override def bulkFlow[D](batchSize: Int)(implicit sWrites: Writer[JsonR, String], docWriter: Writer[D, JsonR], bulkOpWriter: Writer[BulkOpType, JsonR], sReader: Reader[String, JsonR], bReader: Reader[JsonR, BulkResponse[JsonR]], ec: ExecutionContext): Flow[Bulk[D], BulkResponse[JsonR], NotUsed] =
      _this.bulkFlow(Some(name), batchSize)(sWrites, docWriter, bulkOpWriter, sReader, bReader, ec)
    ...
    override def oneBulk[D](bulk: Seq[Bulk[D]])(implicit sWrites: Writer[JsonR, String], docWriter: Writer[D, JsonR], bulkOpWriter: Writer[BulkOpType, JsonR], sReader: Reader[String, JsonR], bReader: Reader[JsonR, BulkResponse[JsonR]], ec: ExecutionContext): Future[BulkResponse[JsonR]] =
      _this.oneBulk(Some(name), bulk: Seq[Bulk[D]])(sWrites, docWriter, bulkOpWriter, sReader, bReader, ec)
    ...
    override def scrollPublisher[Q](query: Q, scroll: String, size: Int = 20)(implicit qWrites: Writer[Q, String], jsonWriter: Writer[Scroll, JsonR], sWriter: Writer[JsonR, String], sReader: Reader[String, JsonR], jsonReader: Reader[JsonR, SearchResponse[JsonR]], ec: ExecutionContext): Publisher[SearchResponse[JsonR]] =
      _this.scrollPublisher(Seq(name), query, scroll, size)(qWrites, jsonWriter, sWriter, sReader, jsonReader, ec)
    ...
    override def scroll[Q](query: Q, scroll: String, size: Int)(implicit qWrites: Writer[Q, String], jsonWriter: Writer[Scroll, JsonR], sWriter: Writer[JsonR, String], sReader: Reader[String, JsonR], jsonReader: Reader[JsonR, SearchResponse[JsonR]], ec: ExecutionContext): Source[SearchResponse[JsonR], NotUsed] =
      _this.scroll(Seq(name), query, scroll, size)(qWrites, jsonWriter, sWriter, sReader, jsonReader, ec)
